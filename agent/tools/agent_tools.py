...
def get_summarize(query:str)->str:
    return rag.rag_summarize(query)

@tool(description="获取指定城市的天气，以消息字符串的形式返回")
def get_weather(city:str)->str:
    return f"城市{city}的天气是晴天,气温为26摄氏度，空气湿度50％，南风1级，AQI21，最近6小时降雨概率极低。"
...
@tool(description="五入参，无返回值，调用后出触发中间件自动为报告生成的场景动态注入上下文信息，为后续提示词切换提供上下文信息")
def fill_context_for_report():
    return "fill_context_for_report已调用"

# if __name__=="__main__":
#     print(get_user_usage_record("1001","2025-01"))
